...
interface PopularVoteIndicatorProps {
  results: {
    Harris: number;
    Trump: number;
    Other: number;
...
export function PopularVoteIndicator({ results }: PopularVoteIndicatorProps) {
  const total = results.Harris + results.Trump + results.Other;
  const harrisPercent = (results.Harris / total) * 100;
  const trumpPercent = (results.Trump / total) * 100;
  
  const harrisDiff = results.Harris - results.Trump;
  const trumpDiff = results.Trump - results.Harris;
  
  const winner = results.Harris > results.Trump ? "Harris" : results.Trump > results.Harris ? "Trump" : "Tie";
  const margin = Math.abs(harrisDiff);
  const winnerPosition = results.Harris > results.Trump ? harrisPercent : 100 - trumpPercent;
